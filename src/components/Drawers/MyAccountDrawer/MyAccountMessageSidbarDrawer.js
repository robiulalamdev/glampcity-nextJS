...
import { setOpenMyAccountMessageSidebar } from '@/Slices/myAccountSlice';

const MyAccountMessageSidbarDrawer = () => {
    const { user } = useContext(AuthContext)
    const { openMyAccountMessageSidebar } = useSelector((state) => state.myAccountSlice)
    ...
    return (
        <Drawer
            drawer={openMyAccountMessageSidebar}
            placement='left'
            className='w-60 h-full bg-white px-2 py-4'
            close={() => dispatch(setOpenMyAccountMessageSidebar(false))}
        >
