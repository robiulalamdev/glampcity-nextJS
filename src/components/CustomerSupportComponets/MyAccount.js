            ...
            <div>
                <div className='w-full h-16 bg-primary flex justify-between items-center rounded-tl-3xl rounded-tr-3xl mt-8 px-4'>
                    <div className='w-full flex items-center justify-start gap-4'>
                        <Image className='w-5 h-5' src={dot} alt="" />
                        <h1 className='text-white md:text-xl'>What can I do if I forgot my password?</h1>
                    </div>
                    <Image className='w-8' src={upArrow} alt="" />
                </div>
                <p className='text-sm md:text-xl text-gray-500 my-2 md:my-6'>Lorem ipsum dolor sit amet consectetur. Diam id risus mi faucibus. Id commodo egestas orci neque eget hendrerit maecenas. Quis pretium habitant risus risus neque mauris ornare. Malesuada.</p>
            </div>
                    ...
                    populerTopics.map(topic => (
                        <div className='h-fit w-full'>
                            <div onClick={() => dispatch(setViewAnsware(viewAnsware ? '' : topic.id))}
                                key={topic.id} className={`w-full h-fit py-2 md:py-4 flex justify-between items-center px-2 md:px-4
                                ${viewAnsware === topic.id ? 'bg-gray-100' : 'hover:bg-gray-100'}`}>
                                <div className='flex justify-start gap-4 items-center'>
                                    <Image className='w-4' src={circle} alt="" />
                                    <h1 className='md:text-xl text-black font-semibold'>{topic.question}</h1>
                                </div>
                                <Image className='w-8' src={downArrow} alt="" />
                            </div>
                            {
                                viewAnsware === topic.id && <div className='bg-gray-100'>
                                    <p className='text-gray-900 md:text-xl px-2 md:px-4 py-2 md:py-3'>{topic.answare}</p>
                                </div>
                            }
                        </div>
                    ))
