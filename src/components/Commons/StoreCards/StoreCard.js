...
const StoreCard = ({ store }) => {
    ...
    return (
        ...
        <Link href={`/stores/${store?.username}`}
            className='cursor-pointer w-fit h-full flex justify-center items-center mx-auto rounded-full border-[12px] md:border-[20px] border-gray-400 hover:border-primary duration-150 relative overflow-hidden'>
            <Image className='w-60 h-60 rounded-full border-[8px] md:border-[14px] border-white' src={profile} alt="" />
