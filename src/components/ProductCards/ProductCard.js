...
const ProductCard = ({ product }) => {
    ...
    return (
        ...
        <div class="border border-gray-600/20" id='box-shadow' >
            <div className='relative w-full h-40 overflow-hidden'>
                <img className='w-full h-40 object-cover' src={product?.image} alt="" />
                {wishlised ?
                    <Image onClick={() => handleWishlistRemove(wishlised?._id)}
                        className='w-8 absolute z-10 top-3 right-3 shadow-xl shadow-blue-400 hover:shadow-green-600 rounded-full hover:scale-125 duration-200' src={love2} alt="" />
                    :
            ...
            <Link href={`/products/${product?.productId ? product?.productId : product?._id}`}
                ...
                <div class="flex flex-col-reverse justify-start md:flex-row md:items-center md:justify-between">
                    <h1 class="flex-grow text-sm font-bold text-gray-800 md:text-[18px]">{product?.title.slice(0, 30)}</h1>
                    <div class="flex items-center gap-2">
                        <svg class="h-5 w-5 text-yellow-500" fill="currentColor" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512"><path d="M316.9 18C311.6 7 300.4 0 288.1 0s-23.4 7-28.8 18L195 150.3 51.4 171.5c-12 1.8-22 10.2-25.7 21.7s-.7 24.2 7.9 32.7L137.8 329 113.2 474.7c-2 12 3 24.2 12.9 31.3s23 8 33.8 2.3l128.3-68.5 128.3 68.5c10.8 5.7 23.9 4.9 33.8-2.3s14.9-19.3 12.9-31.3L438.5 329 542.7 225.9c8.6-8.5 11.7-21.2 7.9-32.7s-13.7-19.9-25.7-21.7L381.2 150.3 316.9 18z"></path></svg>
                        <span class="text-sm text-gray-900">5.0</span>
                    </div>
                </div>
                <p class="text-sm text-gray-500">{product?.description.slice(0, 100) + "..."}</p>
